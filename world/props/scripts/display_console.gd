...
func on_interacted(source):
	if not is_focused and not focused_player:
		toggle_focus(source)
...
func toggle_focus(source):
	...
	camera_lerp_timer = cam_lerp_time - camera_lerp_timer
	...
	if is_focused:
		...
		await get_tree().create_timer(cam_lerp_time * 1.5).timeout
