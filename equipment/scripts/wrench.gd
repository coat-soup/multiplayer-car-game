extends Equipment

@export var heal_amount := 30.0
@export var cooldown := 0.9
@export var range := 5.0
@export var anim : AnimationPlayer

const WRENCH_BONK_PARTICLES = preload("res://vfx/wrench_bonk_particles.tscn")
@onready var audio: AudioStreamPlayer = $Audio

var on_cooldown := false
@onready var ui = get_tree().get_first_node_in_group("ui") as UIManager

func on_triggered():
	if on_cooldown: return
	anim.play("wrench_swing")
	print("wrenching")
	var space_state = get_world_3d().direct_space_state

	var query = PhysicsRayQueryParameters3D.create(held_player.camera.global_position, held_player.camera.global_position - held_player.camera.global_basis.z * range, Util.layer_mask([4]))
	query.exclude = [held_player]

	var result := space_state.intersect_ray(query)
	if result:
		Util.spawn_particles_for_time(result.position, WRENCH_BONK_PARTICLES, get_tree().root, 1.0)
		audio.pitch_scale = randf_range(0.9,1.1)
		audio.play()
		var health = result.collider.get_node_or_null("Health") as Health
		if health:
			print("Trying repair on:" + result.collider.name)
			ui.display_prompt("Health: " + str(min(health.max_health, health.cur_health + heal_amount)))
			health.heal.rpc(heal_amount, "")
		else: print("Didn't hit health")
	else: print("Didn't hit anything.")
	
	on_cooldown = true
	await get_tree().create_timer(cooldown).timeout
	on_cooldown = false
